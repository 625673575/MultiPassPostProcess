...
float noise(in vec3 x)
{
    ...
    vec2 rg = iChannel0.SampleLevel(gSampler, (uv + 0.5) / 256.0, 0.).yx;
#else
...
float noise(in vec2 x)
{
    ...
    return iChannel0.SampleLevel(gSampler, (uv + 118.4) / 256.0, 0.).x;
}

vec4 texcube(Texture2D sam, in vec3 p, in vec3 n)
{
    vec4 x = sam.Sample(gSampler, p.yz);
    vec4 y = sam.Sample(gSampler, p.zx);
    vec4 z = sam.Sample(gSampler, p.xy);
...
static vec3 lig = normalize(vec3(-0.3, 0.4, 0.7));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // ray    
    vec3 rd = mul(cam, normalize(vec3(p.xy, fl)));
    ...
    if (t > 0.0)
    {
        ...
        verde *= iChannel2.Sample(gSampler, pos.xz).xyz;
        col = mix(col, 0.8 * verde, hh);
		
        float vv = smoothstep(0.0, 0.8, nor.y) * smoothstep(0.0, 0.1, pos.y - 0.8);
        verde = vec3(0.2, 0.45, 0.1);
        verde *= iChannel2.Sample(gSampler, 30.0 * pos.xz).xyz;
        verde += 0.2 * iChannel2.Sample(gSampler, 1.0 * pos.xz).xyz;
        vv *= smoothstep(0.0, 0.5, iChannel2.Sample(gSampler, 0.1 * pos.xz + 0.01 * nor.x).x);
