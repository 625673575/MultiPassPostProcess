...
float fbm(vec2 p)
{
    return 0.5000 * iChannel1.Sample(gSampler, p * 1.00).x +
           0.2500 * iChannel1.Sample(gSampler, p * 2.02).x +
           0.1250 * iChannel1.Sample(gSampler, p * 4.03).x +
           0.0625 * iChannel1.Sample(gSampler, p * 8.04).x;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float co = cos(an);
    float si = sin(an);
    p = mul(mat2(co, -si, si, co), p);
    
    // water
    vec2 q = vec2(p.x, 1.0) / p.y;
    q.y -= 0.9 * time;
    vec2 off = iChannel0.Sample(gSampler, 0.1 * q * vec2(1.0, 2.0) - vec2(0.0, 0.007 * iTime)).xy;
    q += 0.4 * (-1.0 + 2.0 * off);
    vec3 col = iChannel0.Sample(gSampler, 0.1 * q * vec2(.5, 8.0) + vec2(0.0, 0.01 * iTime)).zyx;
    col *= 0.4;
    ...
    col += 0.7 * re2 * smoothstep(0.35, 1.0, iChannel1.Sample(gSampler, 0.1 * q * vec2(0.5, 8.0)).x);
    
    // sky
    vec3 sky = vec3(0.0, 0.05, 0.1) * 1.4;
    // stars    
    sky += 0.5 * smoothstep(0.95, 1.00, iChannel1.Sample(gSampler, 0.25 * p).x);
    sky += 0.5 * smoothstep(0.85, 1.0, iChannel1.Sample(gSampler, 0.25 * p).x);
