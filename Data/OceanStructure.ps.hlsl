...
static vec3 qHit, sunDir, smkPos;
static float dstFar, tCur, smkRadEx, smkRadIn, smkPhs, smkHt, tWav;
static int idObj;
static bool isNite;
static const int idBase = 1, idPlat = 2, isShel = 3, idFrm = 4, idDway = 5, idTwr = 6, idBrg = 7, idBrCab = 8, idRdw = 9;
...
float CentStrucDf(vec3 p, float dMin, float ar)
{
    ...
    q = p;
    q.xz = qo;
    q.xy -= vec2(-0.98 * cRad, cLen + 1.2 * doorHt - 9.);
...
vec4 SmokeCol(vec3 ro, vec3 rd, float dstObj)
{
    ...
    if (d < min(dstObj, dstFar))
    {
        ...
        for (int j = 0; j < 150; j++)
        {
            ...
            d += 2.2 * smkRadEx / 150.;
            if (col4.w > 0.99 || d > dstFar)
                break;
        }
    }
    if (isNite)
        col4.rgb *= vec3(0.3, 0.4, 0.3);
    return col4;
}
...
vec4 ObjCol(vec3 n)
{
    vec4 col;
    if (idObj == idBase)
        col = vec4(0.3, 0.4, 0.1, 0.1);
    else if (idObj == idPlat)
        col = vec4(0.4, 0.4, 0.3, 0.1);
    else if (idObj == isShel)
        col = vec4(0.5, 0.5, 0.5, 0.3);
    else if (idObj == idFrm)
        col = vec4(0.8, 0.8, 0.9, 0.5);
    else if (idObj == idDway)
        col = vec4(0.7, 0.8, 0.7, 0.3);
    else if (idObj == idTwr)
        col = vec4(0.7, 0.7, 0.6, 0.3);
    else if (idObj == idBrg)
        col = vec4(1., 0.3, 0.3, 0.3);
    else if (idObj == idBrCab)
        col = vec4(0.9, 0.9, 1., 0.5);
    else if (idObj == idRdw)
        col = vec4(0.4, 0.3, 0.3, 0.1);
    return col;
}
...
vec4 GlowCol(vec3 ro, vec3 rd, float dstObj)
{
    ...
    for (float j = 0.; j < 4.; j++)
    {
        ...
        if (s > 0. && gloDist < dstObj)
            wGlow += 1. - smoothstep(1., 2., sqrt(1. - s * s) * gloDist);
    }
    gloDir = vec3(0., 15.5, 0.) - ro;
    gloDist = length(gloDir);
    s = dot(rd, normalize(gloDir));
    if (s > 0. && gloDist < dstObj)
        wGlow += 1. - smoothstep(2., 3., sqrt(1. - s * s) * gloDist);
...
vec3 ShowScene(vec3 ro, vec3 rd)
{
    ...
    if (dstObj < dstFar)
    {
        ...
        if (idObj == idRdw)
            ...
            vn = VaryNf(2. * floor(16. * qHit), vn, 2.);
        objCol = ObjCol(vn);
        if (isNite)
            col = objCol.rgb * vec3(0.3, 0.35, 0.3) * (0.2 + 0.8 * max(0., vn.y));
        else
            col = objCol.rgb * (0.2 + 0.8 * max(0., max(dot(vn, sunDir), 0.))) +
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    ro = mul(vuMat, vec3(0., 10., -100.));
    rd = mul(vuMat, normalize(vec3(uv, 4.2)));
    sunDir = mul(vuMat, normalize(vec3(1., 1., -1.)));
...
const static float cHashM = 43758.54;
